...
treatments[is.na(treatments)] <- "Empty"
treatments <- make.unique(treatments)
...
variables[is.na(variables)] <- "Empty"
variables <- make.unique(variables)
measures = as.list(conditions_data[7:9,7])$...7
measures[is.na(measures)] <- "Empty"
measures <- make.unique(measures)
...
for(i in seq_along(test_lines_variables)){
  if(is.na(test_lines_variables[i])) next
  values = lines[[i]]
  values = values[!is.na(values)]
  if(length(unique(values)) < 2) next
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "tTest"){if(normality_lines_variables[i] == "normal"){result = t.test(as.numeric(lines[[i]]) ~ lines[["lines"]], var.equal = TRUE, na.action = na.omit)$p.value} else {result = t.test(as.numeric(lines[[i]]) ~ lines[["lines"]], var.equal = FALSE, na.action = na.omit)$p.value}
  ...
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "MannWithney"){result = wilcox.test(as.numeric(lines[[i]]) ~ lines[["lines"]], na.action = na.omit)$p.value
    anova_lines_variables[[i]][[1]] = "A"
    if(result < 0.05){if(mean(na.omit(as.numeric(lines[lines$lines==na.omit(lines_names)[1],i]))) > mean(na.omit(as.numeric(lines[lines$lines==na.omit(lines_names)[2],i])))){
      anova_lines_variables[[i]][[2]] = "B"} else {
        anova_lines_variables[[i]][[1]] = "B"
  ...
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "Tukey"){lines_names_ordered = na.omit(arrange(lines %>%  group_by(lines) %>% summarise(mean = mean(na.omit(as.numeric(!!sym(variables[i]))))), desc(mean))$lines)
  ...
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "Fisher"){lines_names_ordered = na.omit(arrange(lines %>%  group_by(lines) %>% summarise(mean = mean(na.omit(as.numeric(!!sym(variables[i]))))), desc(mean))$lines)
    levels(lines_names_ordered) = lines_names_ordered
    lines_ordered = lines
    lines_ordered$lines = factor(lines_ordered$lines, levels = levels(lines_names_ordered))
    lines_ordered = arrange(lines_ordered, lines_ordered$lines)
    ...
    for(j in c(1:length(letters_anova_ordered))){anova_lines_variables[[i]][[j]] = toupper(letters_anova_ordered[j])}}
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "DunnetT3"){lines_names_ordered = na.omit(arrange(lines %>%  group_by(lines) %>% summarise(mean = mean(na.omit(as.numeric(!!sym(variables[i]))))), desc(mean))$lines)
    levels(lines_names_ordered) = lines_names_ordered
    lines_ordered = lines
    lines_ordered$lines = factor(lines_ordered$lines, levels = levels(lines_names_ordered))
    lines_ordered = arrange(lines_ordered, lines_ordered$lines)
    ...
    letters_anova_ordered = order_letters(na.omit(lines_names), rownames(letters_anova$LetterMatrix), letters_anova$Letters)
    for(j in c(1:length(letters_anova_ordered))){anova_lines_variables[[i]][[j]] = toupper(letters_anova_ordered[[j]])}}
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "GamesHowell"){lines_names_ordered = na.omit(arrange(lines %>%  group_by(lines) %>% summarise(mean = mean(na.omit(as.numeric(!!sym(variables[i]))))), desc(mean))$lines)
    levels(lines_names_ordered) = lines_names_ordered
    lines_ordered = lines
    lines_ordered$lines = factor(lines_ordered$lines, levels = levels(lines_names_ordered))
    lines_ordered = arrange(lines_ordered, lines_ordered$lines)
    ...
    letters_anova_ordered = order_letters(na.omit(lines_names), rownames(letters_anova$LetterMatrix), letters_anova$Letters)
    for(j in c(1:length(letters_anova_ordered))){anova_lines_variables[[i]][[j]] = toupper(letters_anova_ordered[[j]])}}
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "KruskalWallisNemenyi"){lines_names_ordered = na.omit(arrange(lines %>%  group_by(lines) %>% summarise(mean = mean(na.omit(as.numeric(!!sym(variables[i]))))), desc(mean))$lines)
    levels(lines_names_ordered) = lines_names_ordered
    lines_ordered = lines
    lines_ordered$lines = factor(lines_ordered$lines, levels = levels(lines_names_ordered))
    lines_ordered = arrange(lines_ordered, lines_ordered$lines)
    ...
    letters_anova_ordered = order_letters(na.omit(lines_names), rownames(letters_anova$LetterMatrix), letters_anova$Letters)
    for(j in c(1:length(letters_anova_ordered))){anova_lines_variables[[i]][[j]] = toupper(letters_anova_ordered[[j]])}}
  if(!is.na(test_lines_variables[i]) && test_lines_variables[i] == "KruskalWallisDunn"){lines_names_ordered = na.omit(arrange(lines %>%  group_by(lines) %>% summarise(mean = mean(na.omit(as.numeric(!!sym(variables[i]))))), desc(mean))$lines)
    levels(lines_names_ordered) = lines_names_ordered
    lines_ordered = lines
    lines_ordered$lines = factor(lines_ordered$lines, levels = levels(lines_names_ordered))
    lines_ordered = arrange(lines_ordered, lines_ordered$lines)
